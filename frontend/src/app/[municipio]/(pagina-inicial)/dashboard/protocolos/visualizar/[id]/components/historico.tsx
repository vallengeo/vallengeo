...
export function Historico({ protocolo }: HistoricoProps) {
  return (
    <div className="flex-1 bg-white border border-input rounded-3xl p-6 overflow-hidden">
      <h2 className="text-xl font-medium mb-6">Histórico</h2>

      {protocolo.historicos.length ? (
        <div className="relative pl-8">
          <div className="space-y-6">
            {protocolo.historicos.map((historico) => (
              <div key={historico.id}>
                <span
                  className="block bg-[#E3E3E3] size-4 rounded-full absolute left-0 translate-y-1 -translate-x-1.5 z-10"
                ></span>
