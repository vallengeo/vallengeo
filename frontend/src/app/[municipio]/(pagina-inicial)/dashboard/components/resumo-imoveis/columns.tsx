...
export const columns: ColumnDef<ConteudoItem>[] = [
  ...
  {
    accessorKey: "informacaoImovel.tipoUso.nome",
    header: ({ column }) => {
      return (
        <button
          onClick={() => column.toggleSorting(column.getIsSorted() === "asc")}
    ...
    cell: ({ row }) => {
      ...
      switch (row.original.informacaoImovel.tipoUso.nome) {
        case "Casa":
        case "Residencial":
          background = "bg-[#005074]";
          break;
        case "Apartamento":
          background = "bg-[#0EA061]";
          break;
        case "Comercial":
        ...
        case "Indústria":
          ...
          break;
        case "Especial":
          background = "bg-[#6A68F0]";
          break;
      }

      return (
        <div
          className={`text-sm font-light text-white ${background} w-fit px-2 rounded-3xl`}
        >
          {row.original.informacaoImovel.tipoUso.nome}
        </div>
      );
    },
  },
